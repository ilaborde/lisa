...
describe('persistencia de práctica',()=>{it('guarda y restaura con defaults de playback',()=>{let raw:string|null=null;const storage={getItem:()=>raw,setItem:(_key:string,value:string)=>{raw=value}};const session={mode:'guided',genre:'blues',approach:'global',activeChordIndex:0,keepNearby:true,target:'both',globalScale:{root:'A',scaleKey:'minor-blues'},chordConfigs:[],bpm:80,beatsPerBar:4,globalBars:1,loop:true,countIn:0,showNextTarget:true} as PracticeSession;savePracticeSession(session,storage);expect(loadPracticeSession(storage)).toMatchObject({...session,bpm:80,beatsPerBar:4,globalBars:1,loop:true,countIn:0,showNextTarget:true})})});
