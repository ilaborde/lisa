...
type Props={root:string;scaleKey:ScaleKey;labelMode:'notes'|'degrees'|'both';highlightedPositions?:readonly {string:number;fret:number}[]};
const DOTS=[3,5,7,9,12,15,17,19,21];
export function Fretboard({root,scaleKey,labelMode,highlightedPositions=[]}:Props){
 ...
 return <div className="fretboard-component"><div className="fretboard-scroll"><div className="fretboard-stage">
  ...
  <div><div className="fretboard">{[...board].reverse().map((row,i)=><div className="string-row" key={i}>{row.slice(1).map(cell=>{
   ...
   const stringNumber=6-i,sequence=highlightedPositions.findIndex(p=>p.string===stringNumber&&p.fret===cell.fret);
   return <button type="button" key={cell.fret} className="fret-cell" onClick={()=>setSelected(cell)} aria-label={`Cuerda ${cell.string}, traste ${cell.fret}: ${cell.noteName}${cell.degree?`, grado ${cell.degree}`:', fuera de escala'}`}>
    <span className={`marker ${status} ${sequence>=0?'lick-position':''}`}>{sequence>=0?<b className="lick-sequence">{sequence+1}</b>:<>{labelMode!=='degrees'&&<span>{cell.noteName.replace('#','♯').replace('b','♭')}</span>}{labelMode!=='notes'&&cell.degree&&<small>{cell.degree}</small>}</>}</span>
   </button>})}</div>)}</div>
